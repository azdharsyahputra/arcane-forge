...
export default function QuestLog({ quests, onComplete, onUndo, onDelete }: QuestLogProps) {
  return (
    <div className="flex flex-col items-center gap-8">
      ...
      <div className="flex flex-wrap gap-8 justify-center">
        {quests.map((q) => (
          ...
          >
            {q.image && (
              <div className="relative w-[250px] h-[150px] rounded-t-xl overflow-hidden">
                <img src={q.image} alt={q.title} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                  <h3 className="text-2xl font-extrabold text-purple-400 text-center px-2">{q.title}</h3>
                </div>
              </div>
            )}
              ...
              <div className="flex gap-2">
                {!q.completed && (
                  <>
                    <Button
                      variant="accent"
                      className="px-4 py-2 rounded-lg font-semibold tracking-wide shadow-md shadow-purple-600/70
                    ...
                    {onDelete && (
                      <Button
                        variant="accent" // ganti destructive ke accent
                        className="px-4 py-2 rounded-lg font-semibold tracking-wide
                          bg-red-600 text-white hover:bg-red-700 shadow-md shadow-red-600/50 transition-shadow duration-300"
                ...
                {q.completed && onUndo && (
                  <Button
                    variant="accent" // ganti secondary ke accent
                    className="px-4 py-2 rounded-lg font-semibold tracking-wide shadow-md shadow-gray-700/50
                      hover:shadow-gray-800/60 transition-shadow duration-300"
                    onClick={() => onUndo(q.id)}
                  >
                    Undo
                  </Button>
